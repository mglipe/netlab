...
public class Datas {
	...
	public String converte2Banco(Date dt){
	 ...
	 public Date truncaNewDate(Date data){
		    String s_data="";
		    if(data!=null)
		    	s_data = format.format(data);
		    montaDate(s_data);
		   return montaDate(s_data);
		 } 
	...
	return this.getHora()+":"+this.getMinuto();
	 ...
	 public String formataCETUS(Date data){
			String dia = this.getDiaMes(data);
			String mes = this.getMes(data);
			String ano = this.getAno(data);
		 return dia+mes+ano;
	}
	...
	public String getNomeMes(String dt){
	 if ((dt!=null)&&(!dt.equals(""))){		
		String Mes = null;
		try {
		date = format.parse(dt);} catch (Exception e) {e.printStackTrace();}
		cal = Calendar.getInstance();
		cal.setTime(date);
		int mes = cal.get(Calendar.MONTH)+1;
		switch(mes){
	...
	public String getNomeMes(Date da){
			String Mes = null;
			cal = Calendar.getInstance();
			cal.setTime(da);
			int mes = cal.get(Calendar.MONTH)+1;
 ...
 public long compararDatas(Date d1,Date d2){
	   long dt = (d2.getTime() - d1.getTime()) + 3600000;
	   String dat = this.converte2Banco(d1);
	   if(converte2Banco(d1).equals(converte2Banco(d2)))
		   return 0l;
	   return (dt / 86400000L);
	 }
